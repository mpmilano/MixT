...
ssh  -o "UserKnownHostsFile /dev/null"  -o strictHostKeyChecking=no -i MyriaInstances.pem ubuntu@"$foo" sudo /bin/bash vm-actions.sh $i $configuration $write_percent $strong_percent "$foo" $ndebug $strong_target $causal_target_1 $causal_target_2 $num_clients $client_rate $increase_by $increase_delay $test_stop_time $max_threads $first_iter
