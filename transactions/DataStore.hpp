...
namespace myria{
	...
	public:
        ...
        virtual std::unique_ptr<mtl::StoreContext<Level::strong> > begin_transaction(std::unique_ptr<mutils::abs_StructBuilder>&, const std::string& why) = 0;
	...
	public:
		...
		//when the transaction is over.  Do any cleanup you need to do then.
		//the parameters to this function should just be passed directly to TransactionContext's constructor.
        virtual std::unique_ptr<mtl::StoreContext<Level::causal> > begin_transaction(std::unique_ptr<mutils::abs_StructBuilder>&, const std::string& why) = 0;
