...
auto isValid_desugar(Temporary<id,l,T> const * const gt, const CS &cs){
	return make_if(
		make_isValid(
			RefTemporary<ID,get_level<Temp>::value, typename extract_type<Temp>::type,Temp>{*gt}),
		, cs, std::tuple<>());
}
...
struct MutableDeclarationBuilder {
	...
	DeclarationBuilder(const PrevBuilder &pb, const MutDeclarationScope<ID,CS,l,Temp> &d)
	...
	auto operator/(const T &t) const{
		...
		typedef typename _impl_pick_new_type<
			std::decay<*decltype(find_usage<ID>(t))>::type,
			std::decay<*this_decl.gt>::type
			> found_type;
		...
		MutDeclarationScope<ID, decltype(new_cs),new_level,found_type>
			new_decl(this_decl.name,
					 ...
					 new_cs);
		MutDeclarationBuilder<PrevBuilder, ID, decltype(new_cs), new_level, contains_temporary<ID,T>::value || oldb, found_type>
...
struct ImmutableDeclarationBuilder {
	...
	DeclarationBuilder(const PrevBuilder &pb, const ImmutDeclarationScope<ID,CS,l,Temp> &d)
		:prevBuilder(pb),this_decl(d){}
		...
		ImmutDeclarationScope<ID, decltype(new_cs),new_level,found_type>
			new_decl(this_decl.name,
					 (this_decl.gt ?
					  ...
					  make_shared<found_type>(find_usage<ID>(t))),
					 new_cs);
		ImmutDeclarationBuilder<PrevBuilder, ID, decltype(new_cs), new_level, contains_temporary<ID,T>::value || oldb, found_type>
			r(prevBuilder,new_decl);
		return r;
	}
}
...
auto append(const PrevBuilder &pb, const MutVarScopeBegin<ID> &vsb){
	MutDeclarationScope<ID,std::tuple<>,Level::strong,'' > dcl(vsb.name,nullptr,std::tuple<>());
...
auto append(const PrevBuilder &pb, const ImmutVarScopeBegin<ID> &vsb){
	...
	ImmutDeclarationBuilder<PrevBuilder, ID, std::tuple<>,Level::strong, false, std::nullptr_t > db(pb,dcl);
	return db;
}
...
auto append(const MutDeclarationBuilder<PrevBuilder, ID, CS, l, b,temp>  &pb, const VarScopeEnd&){
	static_assert(b,"Error: must use all temporaries you define in a transaction.");
	return append(pb.prevBuilder, pb.this_decl);
}

template<typename PrevBuilder, unsigned long long ID, typename CS, Level l, bool b, template temp>
auto append(const ImmutDeclarationBuilder<PrevBuilder, ID, CS, l, b, temp>  &pb, const VarScopeEnd&){
	static_assert(b,"Error: must use all temporaries you define in a transaction.");
	return append(pb.prevBuilder, pb.this_decl);
}
