...
namespace myria { namespace tracker { 
		...
		auto wrapStore(DS &){
		  ...
		  static const newTomb_t newTomb = [](tracker::Tracker &trk, void* ctx, GDataStore &_ds, Name name, auto &e){
		    auto &ds = dynamic_cast<DS&>(_ds);
		    return std::unique_ptr<LabelFreeHandle<Tracker::Tombstone> >
		    (new Handle<l,Tracker::Tombstone> (ds.template newObject(trk,(mtl::PhaseContext<typename DS::label>*)ctx, name,e)));
		  };
