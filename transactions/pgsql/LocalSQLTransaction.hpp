...
#define log_send(...) ;
  ...
  void prepared(const F &f, LocalSQLConnection_super &sql_conn,
                LocalTransactionNames name, const std::string &stmt, Arg1 &&a1,
                Args &&... args);

  template <typename LocalSQLTransaction>
  typename std::unique_ptr<typename LocalSQLTransaction::SQLConn>
  store_commit(std::unique_ptr<LocalSQLTransaction> o,
  ...
  virtual void select_version(char const *const bytes,
                              mutils::connection &socket) = 0;
  virtual void select_version_data(char const *const bytes,
                                   mutils::connection &socket) = 0;
  virtual void update_data(char const *const bytes,
                           mutils::connection &socket) = 0;
  virtual void initialize_with_id(char const *const bytes,
                                  mutils::connection &socket) = 0;
  virtual void increment(char const *const bytes,
                         mutils::connection &socket) = 0;
  ...
  static void
  indicate_serialization_failure(whendebug(std::ofstream &log_file, )
                                     mutils::connection &socket) {
    char abort{1};
  ...
  static void all_fine(whendebug(std::ofstream &log_file, )
                           mutils::connection &socket) {
    char ok{0};
  ...
  template <typename LocalSQLTransaction>
  auto receiveSQLCommand(std::unique_ptr<LocalSQLTransaction> o,
                         TransactionNames name, char const *const bytes,
                         mutils::connection &socket) {
...
public:
  ...
  void select_version_data_s(std::function<void(long int, long int)> action,
                             Table t, Name id);

  void
  select_version_data_s(std::function<void(long int, mutils::Bytes)> action,
                        Table t, Name id);

  void update_data_s(std::function<void(long int)> action, Table t, Name id,
                     char const *const b);
  ...
  static std::unique_ptr<SQLConn>
      store_abort(std::unique_ptr<LocalSQLTransaction<Level::strong>>);
...
public:
  ...
  static constexpr int group_mapper(int k) {
    ...
    return (k < overflow ? 0 : (k - overflow - 1) / (NUM_CAUSAL_MASTERS)) + 1;
  }

  static constexpr int md(int k) {
    return (k % NUM_CAUSAL_GROUPS == 0 ? NUM_CAUSAL_GROUPS
                                       : k % NUM_CAUSAL_GROUPS);
  }

  static constexpr auto update_data_c_cmd(
      LocalTransactionNames const(&xx)[Table_max * NUM_CAUSAL_GROUPS],
      char const *const set) {
    using namespace std;
    using namespace mutils;
    struct ReturnThis {
      static constexpr CTString vcs1(unsigned int i, unsigned int stop,
                                     unsigned int _k) {
      ...
      static constexpr CTString vcs2(unsigned int i, unsigned int stop) {
        return CTString{} + "vc" + i + "," +
               (i == (stop - 1) ? CTString{} : vcs2(i + 1, stop));
      }

      pair<LocalTransactionNames, CTString>
          ret[Table_max * (NUM_CAUSAL_GROUPS + 1)]{
              pair<LocalTransactionNames, CTString>{LocalTransactionNames::MAX,
                                                    CTString{}}};
    ...
    for (int _t = 0; _t < Table_max; ++_t) {
      ...
      for (int _k = 1; _k < (NUM_CAUSAL_GROUPS + 1); ++_k) {
        ...
        CTString main = CTString{} + "update " + t + " set data = " + set +
                        ReturnThis::vcs1(1, NUM_CAUSAL_GROUPS, _k) + ", lw = " +
                        k + " where id = $1 and index = " + group_mapper(_k) +
  ...
  void select_version_c(
      std::function<void(long int, long int, long int, long int)> action,
      Table t, Name id);

  void select_version_data_c(std::function<void(long int, long int, long int,
                                                long int, long int)> action,
                             Table t, Name id);

  void
  select_version_data_c(std::function<void(long int, long int, long int,
                                           long int, mutils::Bytes)> action,
                        Table t, Name id);
  ...
  void initialize_with_id_c(Table t, int k, Name id,
                            const std::array<int, NUM_CAUSAL_GROUPS> &ends,
                            const Blob &b);
  ...
  static std::unique_ptr<SQLConn>
      store_abort(std::unique_ptr<LocalSQLTransaction<Level::causal>>);
