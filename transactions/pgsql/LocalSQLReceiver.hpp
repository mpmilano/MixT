	...
	namespace pgsql {
		namespace local {
			...
			class SQLReceiver{
				...
				using action_t = typename mutils::receiver::action_t;
				using sizes_t = std::vector<std::size_t>;

				static std::function<sizes_t (void**, mutils::connection&)> new_connection(){
					return [db_connection = std::make_unique<LocalSQLConnection<l> >()]
						(void* data, mutils::connection& conn) {
						char* _data = (char*) data;
						if (!db_connection->current_trans) {
							assert(_data[0] == 4);
							db_connection->current_trans
								= std::make_unique<LocalSQLTransaction<l> >(*db_connection);
