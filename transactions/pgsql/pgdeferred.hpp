...
struct deferred_action {
  ...
  deferred_action(std::function<void(pgresult)> on_complete,
                  const std::string &query_str, std::function<void()> query)
      : on_complete(on_complete), query_str(query_str), query(query) {}
