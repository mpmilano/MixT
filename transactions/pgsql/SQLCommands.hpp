	...
	namespace cmds {
		...
		constexpr auto update_data_c_cmd(TransactionNames const (&xx)[Table_max * NUM_CAUSAL_GROUPS],
			...
			struct ReturnThis{
				...
				pair<TransactionNames,CexprString> ret[Table_max * (NUM_CAUSAL_GROUPS+1)]
					{pair<TransactionNames,CexprString>{TransactionNames::MAX,CexprString{} }};
		...
		template<typename T, typename Blob>
		auto update_data_c(T &trans, Table t, int k, Name id, const std::array<long long,NUM_CAUSAL_GROUPS> &ends, const Blob &b){
		...
		template<typename T>
		auto increment_c(T &trans, Table t, int k, Name id, const std::array<long long,NUM_CAUSAL_GROUPS> &ends){
			static const constexpr TransactionNames tnames[NUM_CAUSAL_GROUPS * Table_max] = {
		...
		template<typename T, typename Blob>
		void initialize_with_id_c(T &trans, Table t, int k, Name id, const std::array<long long,NUM_CAUSAL_GROUPS> &ends, const Blob& b){
			assert(k > 0);
		...
		template<typename T, typename Blob>
		auto update_data(Level l, T &tran, Table t, int k, Name id, const std::array<long long,NUM_CAUSAL_GROUPS> &ends, const Blob& b){
		...
		template<typename T, typename Blob>
		void initialize_with_id(Level l, T &tran, Table t, int k, Name id, const std::array<long long,NUM_CAUSAL_GROUPS> &ends, const Blob& b){
		...
		template<typename T>
		auto increment(Level l, T& tran, Table t, int k, Name id, const std::array<long long,NUM_CAUSAL_GROUPS> &ends){
