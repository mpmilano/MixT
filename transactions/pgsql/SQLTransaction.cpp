...
namespace myria{ namespace pgsql {
		...
		SQLTransaction::SQLTransaction(SQLStore_impl &parent, GDataStore& store, LockedSQLConnection c, std::string why)
			:gstore(store),parent(parent),sql_conn(std::move(c)),conn_lock(
				[](auto& l) -> auto& {
					assert(l.try_lock());
					l.unlock();
					return l;
