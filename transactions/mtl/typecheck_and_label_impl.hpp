...
constexpr auto _typecheck(env, parse_phase::Expression<parse_phase::BinOp<op, L, R>>)
{
  ...
  using label = raw_label_min<typename next_l::label, typename next_r::label>;
...
constexpr auto _typecheck(type_environment<label_env, Env...>, parse_phase::Expression<parse_phase::IsValid<Expr>>)
{
  ...
  using new_label = raw_label_min<raw_label_min<label_env, ptr_label>, typename binding_expr::yield::label>;
...
constexpr auto _typecheck(old_env, parse_phase::Statement<parse_phase::Let<parse_phase::Binding<Name, Expr>, Body>>)
{
  ...
  return Statement<raw_label_min<typename binding_expr::label, typename old_env::pc_label>, Let<next_binding, next_body>>{};
}
...
constexpr auto _typecheck(type_environment<label_env, Env...>, parse_phase::Statement<parse_phase::LetRemote<parse_phase::Binding<Name, Expr>, Body>>)
{
  ...
  using new_env = type_environment<raw_label_min<label_env, ptr_label>, Env...,
                                   type_binding<Name, handle, raw_label_min<ptr_label, typename handle::label>, type_location::remote> >;
  using next_body = DECT(typecheck<seqnum + 1, depth + 1>(new_env{}, Body{}));
  using next_binding = Binding<typename handle::label, typename handle::type, Name, binding_expr>;
  return Statement<raw_label_min<label_env, typename handle::label>, LetRemote<next_binding, next_body>>{};
}
...
struct handle_operations{
	...
	constexpr static auto handle_operation(std::true_type* /*won't use result*/, type_environment<label_env, Env...> a){
		...
		using oper_label = raw_label_min<ptr_label,raw_label_min<label_env, arguments_label_min> >;
	...
	constexpr static auto handle_operation(std::false_type* /*might use result*/, type_environment<label_env, Env...> a){
		using old_env = DECT(a);
		using operation_execution_label = raw_label_min<ptr_label,raw_label_min<label_env, arguments_label_min> >;
...
constexpr auto _typecheck(old_env, parse_phase::Operation<oper_name, Hndl, parse_phase::operation_args_exprs<>,parse_phase::operation_args_varrefs<var_args...> >)
{
  ...
  // we dereference the pointer, which is an influencing action.  Reduce the label
  // of the environment if needed.
  using arguments_label_min = raw_label_min<typename handle::label, /*the duplication is on purpose*/ typename handle::label, typename DECT(typecheck<seqnum,depth+1>(old_env{},var_args{}))::label...>;
...
constexpr auto _typecheck(type_environment<pc_label, Env...>, parse_phase::Statement<parse_phase::Assignment<Var, Expr>>)
{
  ...
  return Statement<raw_label_min<raw_label_min<typename new_expr::label, typename new_var::label>, pc_label>, Assignment<new_var, new_expr>>{};
}
...
constexpr auto _typecheck(type_environment<pc_label, Env...>, parse_phase::Statement<parse_phase::While<condition, body>>)
{
  ...
  using new_label = raw_label_min<typename next_condition::label, pc_label>;
  using new_env = type_environment<new_label, Env...>;
  using next_body = DECT(typecheck<seqnum + 1, depth + 1>(new_env{}, body{}));
  return Statement<raw_label_min<pc_label, typename next_condition::label>, While<next_condition, next_body>>{};
}
...
constexpr auto _typecheck(type_environment<pc_label, Env...>, parse_phase::Statement<parse_phase::If<condition, then, els>>)
{
  ...
  return Statement<raw_label_min<pc_label, typename next_condition::label>, If<next_condition, next_then, next_else>>{};
}
