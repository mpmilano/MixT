...
template <typename l, typename candidates, typename sub_map, char... var>
auto _recollapse(typename AST<l>::template Statement<typename AST<l>::template IncrementRemoteOccurance<String<var...>>> a)
{
  static_assert(!candidates::template contains<String<var...>>());
  return a;
}
...
constexpr auto anorm_names(mutils::typeset<vars...>)
{
  ...
  return anorm_names2(typelist<>{}, typeset<typename vars::name...>{});
	...
	constexpr auto recollapse_phase(runnable_transaction::phase<id,l, AST, reqs, provides, owns, passthrough>)
{
  ...
  return phase<id,l, new_ast, reqs, provides, DECT(without_names(names{}, owns{})), passthrough>{};
}
}
