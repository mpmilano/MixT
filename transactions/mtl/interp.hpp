...
namespace runnable_transaction {
	...
	auto interp2(mutils::DeserializationManager* dsm, mutils::connection& c, transaction<phase1>*, store& s)
	{
		return remote_interp<phase1>(dsm,c, s);
	}
	
	template <typename store, typename phase1, typename phase2, typename... phase>
	auto interp2(mutils::DeserializationManager* dsm, mutils::connection& c, transaction<phase1, phase2, phase...>*, store& s)
	{
	...
	auto interp3(mutils::DeserializationManager* dsm, mutils::connection& c, void*, split* np, store& s){
		interp2(dsm,c, np, s);
	}

	template <typename ptr, typename split, typename store>
	auto interp3(mutils::DeserializationManager* dsm, mutils::connection& c, ptr*, split* np, store& s, std::enable_if_t<!std::is_void<ptr>::value >* = nullptr){
		auto ret = interp2(dsm,c, np, s);
		return ret;
	}

	template <typename split, typename... required>
	auto begin_interp(mutils::DeserializationManager* dsm, mutils::connection& c, required... vals)
	{
