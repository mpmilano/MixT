...
void update_strong_clock(pqxx::connection& conn_strong, std::array<long long,4>& tmp){
...
void select_strong_clock(pqxx::connection& conn_strong, std::array<long long,4>& arr){
	pqxx::work trans_strong(conn_strong);
	trans_strong.exec("set search_path to \"BlobStore\",public");
...
void verify_strong_clock(pqxx::connection& conn_strong, std::array<long long,4>& tmp){
	std::array<long long,4> arr;
